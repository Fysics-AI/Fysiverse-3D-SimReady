...
def make_stage_specs(session_dir: Path, force_linked_gravity: bool) -> list[StageSpec]:
    ...
    report = read_json(result_dir / "sam3d_moge_report.json")
    report_stages = ((report.get("outputs") or {}).get("stages") or {}) if report else {}

    def stage_source(key: str, default: Path) -> Path:
        stage = report_stages.get(key) or {}
        ...
        if key == "optimized":
            ...
            if not default.is_file() and fallback.is_file():
                return fallback
        return default

    optimized_stage = report_stages.get("optimized") or {}
    optimized_fallback_reason = optimized_stage.get("fallback_reason")
    optimized_title = "04 Differentiable rendering refinement"
    ...
    if optimized_fallback_reason:
        optimized_title = "04 Differentiable rendering refinement fallback"
        optimized_description = (
            "Differentiable rendering refinement was not accepted for this session, so the diagnostic reuses the recorded fallback stage."
        )
    return [
        StageSpec(
            "raw",
            "01 SAM3D raw",
            stage_source("raw", result_dir / "sam3d.blend"),
        ...
        StageSpec(
            ...
            stage_source("rotated", result_dir / "sam3d_moge_rotated.blend"),
        ...
        StageSpec(
            "grounded",
            "03 Grounded to z=0",
            stage_source("grounded", result_dir / "sam3d_moge_grounded.blend"),
        ...
        StageSpec(
            "optimized",
            optimized_title,
            stage_source("optimized", result_dir / "sam3d_moge_optimized.blend"),
            optimized_description,
            "If masks align but 3D becomes implausible, inspect object pose optimization constraints, accept criteria, and fallback reason.",
        ),
        StageSpec(
            "separated",
            "05 Scene-graph + convex separation",
            stage_source("separated", result_dir / "sam3d_moge_separated.blend"),
