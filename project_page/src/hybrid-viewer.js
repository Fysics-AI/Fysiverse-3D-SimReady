...
class HybridSceneViewer {
  ...
  patchHybridRenderOrder() {
    const viewer = this.viewer;
    if (!viewer || viewer.__fysicsHybridRenderPatched) {
      return;
    }

    const nativeRender = viewer.render.bind(viewer);
    viewer.render = function renderHybridScene() {
      nativeRender();
      ...
      if (
        ...
        || !hasVisibleSceneContent(this.threeScene)
      ) {
        return;
      }
    ...
    viewer.__fysicsHybridRenderPatched = true;
  }
...
function hasVisibleSceneContent(scene) {
  return Boolean(scene?.children?.some((child) => child.visible));
}
