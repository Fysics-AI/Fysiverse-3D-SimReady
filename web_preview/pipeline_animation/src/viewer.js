...
function shouldIgnoreStoredCamera() {
  const params = new URLSearchParams(window.location.search);
  const reset = params.get("resetCamera") || "";
  const cameraOverride = params.get("cameraOverride") || "";
  return /^(1|true|yes)$/i.test(reset) || /^(0|false|off|manifest|fit)$/i.test(cameraOverride);
}
