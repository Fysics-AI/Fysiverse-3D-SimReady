...
```text
http://<host>:8075/web_preview/pipeline_animation/index.html?session=<session_id>&cameraOverride=manifest
```

Do not open `animation_manifest.json` directly; it is the data file consumed by
the preview viewer. The viewer can also load a manifest from the `manifest`
query parameter. Use the packaged release manifest path when sharing a scene:
