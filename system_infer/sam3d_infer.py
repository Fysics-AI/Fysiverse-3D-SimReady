...
DEFAULT_SAM3D_ROOT = WORKFLOW_ROOT / "third_party" / "SAM3D"
SAM3D_ROOT = Path(os.environ.get("SAM3D_ROOT", str(DEFAULT_SAM3D_ROOT)))
...
def _resolve_sam3d_root() -> Path:
    return Path(os.environ.get("SAM3D_ROOT", str(DEFAULT_SAM3D_ROOT))).expanduser().resolve()


def _resolve_sam3d_env_dir() -> Path:
    return Path(os.environ.get("SAM3D_ENV_DIR", os.environ.get("CONDA_PREFIX", sys.prefix))).expanduser().resolve()
...
class SAM3DInfer:
    ...
    def __init__(
        ...
        **_: Any,
        ...
        self.root = Path(root) if root is not None else _resolve_sam3d_root()
        self.script = WORKFLOW_ROOT / "system_infer" / "sam3d_cached_runner.py"
        self.env_dir = Path(env_dir) if env_dir is not None else _resolve_sam3d_env_dir()
