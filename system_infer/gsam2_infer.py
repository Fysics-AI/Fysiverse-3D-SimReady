...
DEFAULT_ROOT = Path(__file__).resolve().parents[1] / "third_party" / "Grounded-SAM-2"
ROOT = Path(os.environ.get("GSAM2_ROOT", str(DEFAULT_ROOT)))
SAM2_MODEL_CONFIG = "configs/sam2.1/sam2.1_hiera_l.yaml"

if os.environ.get("GSAM2_TRANSFORMERS_OFFLINE", "1").strip().lower() not in {"0", "false", "no", "off"}:
...
def _resolve_root() -> Path:
    return Path(os.environ.get("GSAM2_ROOT", str(DEFAULT_ROOT))).expanduser().resolve()
...
def _patch_transformers_bert_head_mask() -> None:
    """Restore BertModel mask APIs expected by older GroundingDINO code."""
    try:
        import torch
        from transformers import BertModel
    except Exception:
        return
    if getattr(BertModel, "_fysiverse_grounding_dino_mask_api_patch", False):
        return

    original_get_extended_attention_mask = BertModel.get_extended_attention_mask

    def model_dtype(self: Any, fallback: Any) -> Any:
    ...
    def get_extended_attention_mask(self: Any, attention_mask: Any, input_shape: Any, device_or_dtype: Any = None, dtype: Any = None) -> Any:
        if dtype is None and isinstance(device_or_dtype, torch.dtype):
    ...
    def get_head_mask(self: Any, head_mask: Any, num_hidden_layers: int, is_attention_chunked: bool = False) -> Any:
        ...
        if head_mask.dim() == 1:
            ...
            raise ValueError(f"head_mask.dim must be 1, 2, or 5, got {head_mask.dim()}")
        if is_attention_chunked:
            head_mask = head_mask.unsqueeze(-1)
        return head_mask.to(dtype=model_dtype(self, head_mask.dtype))
    ...
    BertModel._fysiverse_grounding_dino_mask_api_patch = True


def _transformers_offline_enabled() -> bool:
    return os.environ.get("GSAM2_TRANSFORMERS_OFFLINE", "1").strip().lower() not in {"0", "false", "no", "off"}


def _patch_grounding_dino_text_encoder_loading() -> None:
    """Make GroundingDINO load BERT from the local cache when offline mode is requested."""
    if not _transformers_offline_enabled():
        return
    try:
        from transformers import AutoTokenizer, BertModel, RobertaModel
        from grounding_dino.groundingdino.util import get_tokenlizer
    except Exception:
        return
    if getattr(get_tokenlizer, "_fysiverse_local_files_patch", False):
        return

    def normalize_text_encoder_type(text_encoder_type: Any) -> Any:
        if not isinstance(text_encoder_type, str):
            if hasattr(text_encoder_type, "text_encoder_type"):
                text_encoder_type = text_encoder_type.text_encoder_type
            elif hasattr(text_encoder_type, "get") and text_encoder_type.get("text_encoder_type", False):
    ...
    def get_tokenizer_local(text_encoder_type: Any) -> Any:
        text_encoder_type = normalize_text_encoder_type(text_encoder_type)
        print(f"final text_encoder_type: {text_encoder_type}")
        return AutoTokenizer.from_pretrained(text_encoder_type, local_files_only=True)

    def get_language_model_local(text_encoder_type: Any) -> Any:
        text_encoder_type = normalize_text_encoder_type(text_encoder_type)
    ...
    get_tokenlizer.get_tokenlizer = get_tokenizer_local
    get_tokenlizer.get_pretrained_language_model = get_language_model_local
    get_tokenlizer._fysiverse_local_files_patch = True


def _patch_grounding_dino_deformable_attention() -> None:
    """Use GroundingDINO's PyTorch attention fallback when the C++ op is unavailable."""
    try:
        import torch
    ...
    attn_fn = getattr(ms_deform_attn, "MultiScaleDeformableAttnFunction", None)
    if attn_fn is None or getattr(attn_fn, "_fysiverse_pytorch_fallback_patch", False):
        return

    original_apply = attn_fn.apply
    ...
    def pytorch_msda(
        ...
        value_list = value.split([height * width for height, width in shapes], dim=1)
        ...
        for level, (height, width) in enumerate(shapes):
            value_l = (
                ...
                .flatten(2)
                .transpose(1, 2)
                .reshape(bs * num_heads, embed_dims, height, width)
            )
    ...
    def apply_with_fallback(
        ...
        force_fallback = env_flag("GSAM2_FORCE_GDINO_PYTORCH_OP", False)
        has_extension = hasattr(ms_deform_attn, "_C") and getattr(ms_deform_attn, "_C") is not None
        if has_extension and not force_fallback:
            try:
            ...
            except (ImportError, NameError, RuntimeError) as exc:
                message = str(exc)
                if "_C" not in message and "ms_deform_attn" not in message and "undefined symbol" not in message:
                    raise
        return pytorch_msda(value, value_spatial_shapes, sampling_locations, attention_weights)

    attn_fn.apply = staticmethod(apply_with_fallback)
    attn_fn._fysiverse_pytorch_fallback_patch = True
...
class GSAM2Infer:
    def __init__(self, gpu: Optional[Any] = None, **_: Any):
        ...
        _ensure_root_on_path(self.root)
        self.sam2_checkpoint = self.root / "checkpoints" / "sam2.1_hiera_large.pt"
        self.grounding_dino_config = self.root / "grounding_dino" / "groundingdino" / "config" / "GroundingDINO_SwinT_OGC.py"
        self.grounding_dino_checkpoint = self.root / "gdino_checkpoints" / "groundingdino_swint_ogc.pth"
        for path, label in (
            (self.root, "GSAM2_ROOT"),
            (self.sam2_checkpoint, "SAM2 checkpoint"),
            (self.grounding_dino_config, "GroundingDINO config"),
            (self.grounding_dino_checkpoint, "GroundingDINO checkpoint"),
        ):
            if not path.exists():
                raise FileNotFoundError(f"{label} not found: {path}")

        import torch
        _patch_transformers_bert_head_mask()
        import grounding_dino.groundingdino.datasets.transforms as grounding_transforms
        _patch_grounding_dino_text_encoder_loading()
        _patch_grounding_dino_deformable_attention()
        ...
        self.sam2_predictor = SAM2ImagePredictor(
            build_sam2(SAM2_MODEL_CONFIG, str(self.sam2_checkpoint), device=self.device)
        )
        self.grounding_model = load_model(
            model_config_path=str(self.grounding_dino_config),
            model_checkpoint_path=str(self.grounding_dino_checkpoint),
            device=self.device,
        )
