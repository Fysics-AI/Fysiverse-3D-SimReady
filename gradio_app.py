...
class InteractivePipeline:
    ...
    def create_session_from_upload(
        self, image: Any
    ) -> tuple[dict[str, Any], str, str, Any, Any, Any, Any, str, str, Any, Any, Any]:
    ...
    def run_models(self, state: dict[str, Any], models: list[str]) -> tuple[dict[str, Any], str, list[str], Any, Any]:
        if not state:
            raise gr.Error("Upload an image first.")
        ...
        return state, log, files, gr.update(value=scene_graph_vis if Path(scene_graph_vis).is_file() else None), diagnostic_blend
    ...
    with gr.Blocks(title="Image to 3D Scene Pipeline") as demo:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                input_image = gr.Image(type="numpy", label="Input image", height=460, buttons=[], sources=["upload", "clipboard"])
                annotation_overlay = gr.Image(type="numpy", label="Annotation preview", height=260, format="png", image_mode="RGBA", visible=False, buttons=[])
                ...
                with gr.Accordion("Ground prompt", open=True):
                    ground_prompt_image = gr.Image(type="numpy", label="Ground point prompt", height=260, buttons=[])
            ...
            with gr.Column(scale=1):
                seg_preview = gr.Image(type="filepath", label="Segmentation preview", height=460)
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                scene_graph_preview = gr.Image(type="filepath", label="Scene graph preview", height=360, buttons=["download", "fullscreen"])
        with gr.Row():
            with gr.Column(scale=1):
        ...
        input_image.upload(
            ...
            [input_image],
            [state, session_status, seg_preview, annotation_overlay, ground_prompt_image, seg_files, output_files, output_log, scene_graph_status, scene_graph_preview, scene_graph_files, infer_btn],
            queue=False,
        )
        ...
        infer_btn.click(gradio_logged(app.run_models), [state, models], [state, output_log, output_files, scene_graph_preview, pipeline_diagnostic_blend])
