...
def setup_runtime_env(args: argparse.Namespace) -> None:
    ...
    os.environ.setdefault("SAM3D_ENV_DIR", os.environ.get("CONDA_PREFIX", sys.prefix))
...
def run_sam3d(args: argparse.Namespace, session: dict[str, Any]) -> dict[str, Any]:
    ...
    infer = SAM3DInfer(
        ...
        env_dir=os.environ.get("CONDA_PREFIX", sys.prefix),
...
def run_optional_exports(args: argparse.Namespace, session: dict[str, Any], manifest_path: Path) -> None:
    if not args.skip_diagnostic:
        ...
        run_command(cmd, timeout=args.diagnostic_timeout)
        diag = Path(session["result_dir"]) / "pipeline_stage_diagnostic" / "pipeline_stage_diagnostic.blend"
        data = read_json(manifest_path)
        data.setdefault("outputs", {})["pipeline_stage_diagnostic_blend"] = str(diag) if diag.is_file() else None
    ...
    if not args.skip_web_assets:
        ...
        run_command(cmd, timeout=args.web_assets_timeout)
    ...
    if not args.skip_web_animation:
        ...
        run_command(cmd, timeout=args.web_animation_timeout)
...
def run_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "final_manifest": str(manifest_path),
        "final_blend": (read_json(manifest_path).get("outputs") or {}).get("final_blend"),
        "release_package": str(package_dir) if package_dir else None,
        "files": collect_existing([manifest_path, (read_json(manifest_path).get("outputs") or {}).get("final_blend")]),
    }
